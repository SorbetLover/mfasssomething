...
class HaxeMenu extends FlxState
{
	...
	override public function create()
	{
		...
		{
			FlxTween.tween(sprite2, {"scale.x": 1, "scale.y": 1}, 0.3, {ease: FlxEase.backOut});
		});
		new FlxTimer().start(0.58, function(tmr:FlxTimer)
		{
			FlxTween.tween(sprite3, {"scale.x": 1, "scale.y": 1}, 0.3, {ease: FlxEase.backOut});
		});
		new FlxTimer().start(0.74, function(tmr:FlxTimer)
		{
			FlxTween.tween(sprite4, {"scale.x": 1, "scale.y": 1}, 0.3, {ease: FlxEase.backOut});
		});
		new FlxTimer().start(0.88, function(tmr:FlxTimer)
		{
			FlxTween.tween(sprite5, {"scale.x": 1, "scale.y": 1}, 0.3, {ease: FlxEase.backOut});
		});
